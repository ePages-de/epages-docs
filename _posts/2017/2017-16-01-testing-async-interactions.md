...
See the [full test source](https://github.com/mduesterhoeft/testing-asynchonous-interactions/blob/master/checkout-service/src/test/java/com/epages/checkout/ProductSubscriberHardcodedPayloadTest.java) for on how this is done.
...
Spring Cloud Contract is integrating with Spring Integration, Spring Cloud Stream, Apache Camel, and plain [Spring Amqp](https://cloud.spring.io/spring-cloud-contract/spring-cloud-contract.html#_stub_runner_spring_amqp).
...
**This shields us from accidental incompatible contract-relevant changes**.
...
Now all we need to do is annotate our improved test with `@AutoConfigureStubRunner` and use the `StubTrigger` to send the stub message in.
