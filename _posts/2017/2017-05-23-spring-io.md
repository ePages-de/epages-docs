---
layout: post
title: "Spring I/O 2017 - What it really was like"
...
For a long time I had [jOOQ](http://www.jooq.org/) on the list of frameworks I wanted to look at. So it came in handy that [Michael Simons](https://twitter.com/rotnroll666) gave a talk about using it together with Spring Boot.
...
The main use case to apply jOOQ mentioned by Micheal are:
...
So next time you put a 10 line native SQL query in to a String in your JPA repository, think twice and give jOOQ a try!
...
And everybody who ever attended a talk by infamous Josh Long knew what was coming.
...
Their talk gave a very good introduction about how Spring 5 is supporting reactive programming. It is building on Project Reactor to support message-driven, elastic, resilient, and responsive services.
...
In the first part of his talk Sébastien gave a short introduction into [Kotlin](https://kotlinlang.org/). For me many language features looked familiar because of their similarity to Scala:
...
So it is supposed to be quite easy to adopt while still using the full power of Spring Boot and Spring Cloud.
...
I exactly did that after the talk and tried to explore Kotlin and the Spring 5 reactive features a little.
...
Strategic Design identifies _Bounded Contexts_ and _Context Maps_. Each bounded context contains models and maybe other (embedded) contexts. Context maps describe the contact or relationship between models / contexts.
...
This fast speaker showed to us a _beer recommender web service_ mounted as an Angular standalone client app making cross-origin requests to access a Spring Boot back-end API (with Spring MVC and Spring Data REST technologies).
Does this front-end / back-end split sound familiar to you ?.

Get [Bootiful Development with Spring Boot and Angular](http://developer.okta.com/blog/2017/04/26/bootiful-development-with-spring-boot-and-angular) workshop coding.

## It's a wrap

Wrapping it up I can say that I very much enjoyed especially the second day of the conference
...
For people experienced with spring, spring-boot and also spring-cloud there might come across a lot of already known topics.

And of course Barcelona is a beautiful city that has a lot to offer beyond the content of the conference.
